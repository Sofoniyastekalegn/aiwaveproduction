...
import {
  Waves, Bot, Zap, Shield, Target, Mail,
  Phone, Workflow, Activity, Search, Bell, Loader2,
} from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    if (!authLoading && isDashboard && !session) {
      navigate('/');
      setAuthModalOpen(true);
      setAuthModalView('signin');
    }
  }, [authLoading, isDashboard, session]);
    ...
    return (
      <div className="flex bg-[#020617] text-white min-h-screen">
        <Sidebar
          ...
          user={{ name: displayName, avatarInitial }}
        />
  ...
  return (
    <div className="bg-[#020617] text-white overflow-hidden scroll-smooth">
      <Navbar
        onNavigate={(page) => navigate(page === 'dashboard' ? '/dashboard' : '/')}
        currentPage={isDashboard ? 'dashboard' : 'landing'}
        onOpenAuth={openAuth}
      />
            ...
            <div className="grid md:grid-cols-3 gap-8">
              {[
                ...
                <div key={i} className={cn(
                  ...
                  plan.featured ? 'glass-dark border-blue-500 shadow-2xl shadow-blue-500/10 scale-105 z-10' : 'glass border-white/5 hover:border-white/20'
                )}>
                  {plan.featured && (
                    <div className="absolute top-0 right-0 bg-blue-600 text-white text-[10px] font-bold px-4 py-1.5 uppercase tracking-widest rounded-bl-xl">
                      Most Popular
                    </div>
                  )}
          ...
          <div className="relative glass-dark rounded-[40px] border border-white/5 py-24 px-8 overflow-hidden text-center">
            ...
            <div className="relative z-10 max-w-2xl mx-auto space-y-8">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                <button onClick={() => openAuth('signup')} className="btn-primary px-10 py-4 text-lg w-full sm:w-auto">Start Free Trial</button>
