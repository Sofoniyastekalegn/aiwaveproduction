...
export default function Navbar({ onNavigate, currentPage, onOpenAuth }: NavbarProps) {
      ...
      {isOpen && (
        <div className="md:hidden glass border-b border-white/10 px-4 pt-2 pb-6 flex flex-col gap-4 animate-in slide-in-from-top duration-300">
          ...
          <div className="flex flex-col gap-2 pt-4">
            <button onClick={() => { onOpenAuth('signin'); setIsOpen(false); }} className="btn-secondary w-full">Sign In</button>
            <button onClick={() => { onOpenAuth('signup'); setIsOpen(false); }} className="btn-primary w-full">Start Free Trial</button>
          </div>
        </div>
      )}
    </nav>
